# 1 "/Users/foxacid/ghq/github.com/foxacid7cd/Nims/Targets/libNims/src/nvim/grid.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 400 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/Users/foxacid/ghq/github.com/foxacid7cd/Nims/Targets/libNims/src/nvim/grid.c" 2
# 14 "/Users/foxacid/ghq/github.com/foxacid7cd/Nims/Targets/libNims/src/nvim/grid.c"
...
# 5 "/Users/foxacid/ghq/github.com/foxacid7cd/Nims/Targets/libNims/src/nvim/arabic.h" 2
# 15 "/Users/foxacid/ghq/github.com/foxacid7cd/Nims/Targets/libNims/src/nvim/grid.c" 2
...
# 8 "/Users/foxacid/ghq/github.com/foxacid7cd/Nims/Targets/libNims/src/nvim/grid.h" 2
...
# 16 "/Users/foxacid/ghq/github.com/foxacid7cd/Nims/Targets/libNims/src/nvim/grid.c" 2
...
# 17 "/Users/foxacid/ghq/github.com/foxacid7cd/Nims/Targets/libNims/src/nvim/grid.c" 2
...
# 19 "/Users/foxacid/ghq/github.com/foxacid7cd/Nims/Targets/libNims/src/nvim/grid.c" 2
...
# 39 "/Users/foxacid/ghq/github.com/foxacid7cd/Nims/Targets/libNims/src/nvim/grid.c"
...
void grid_clear_line(ScreenGrid *grid, size_t off, int width, _Bool valid)
{
  for (int col = 0; col < width; col++) {
    schar_from_ascii(grid->chars[off + (size_t)col], ' ');
  }
  int fill = valid ? 0 : -1;
  (void)__builtin___memset_chk (grid->attrs + off, fill, (size_t)width * sizeof(sattr_T), __builtin_object_size (grid->attrs + off, 0));
}

void grid_invalidate(ScreenGrid *grid)
{
  (void)__builtin___memset_chk (grid->attrs, -1, sizeof(sattr_T) * (size_t)grid->rows * (size_t)grid->cols, __builtin_object_size (grid->attrs, 0));
}
...
void grid_getbytes(ScreenGrid *grid, int row, int col, char_u *bytes, int *attrp)
{
  ...
  if (grid->chars != ((void*)0) && row < grid->rows && col < grid->cols) {
...
static ScreenGrid *put_dirty_grid = ((void*)0);
static int put_dirty_row = -1;
static int put_dirty_first = 2147483647;
...
void grid_puts_line_start(ScreenGrid *grid, int row)
{
  int col = 0;
  grid_adjust(&grid, &row, &col);
  (__builtin_expect(!(put_dirty_row == -1), 0) ? __assert_rtn(__func__, "grid.c", 169, "put_dirty_row == -1") : (void)0);
  put_dirty_row = row;
  put_dirty_grid = grid;
}
...
{
  (__builtin_expect(!(put_dirty_row == row), 0) ? __assert_rtn(__func__, "grid.c", 176, "put_dirty_row == row") : (void)0);
  ...
  if (grid->attrs[off] != attr || schar_cmp(grid->chars[off], schar) || rdb_flags & 0x008) {
    ...
    put_dirty_first = ((put_dirty_first) < (col) ? (put_dirty_first) : (col));

    put_dirty_last = ((put_dirty_last) > (col + 1) ? (put_dirty_last) : (col + 1));
  }
}
...
void grid_puts_len(ScreenGrid *grid, char *text, int textlen, int row, int col, int attr)
{
  ...
  if (grid != &default_grid && col == 0 && grid_invalid_row(grid, row)) {
    ...
    put_dirty_first = -1;
    put_dirty_last = ((put_dirty_last) > (1) ? (put_dirty_last) : (1));
  }
  ...
  while (col < grid->cols
    ...
    if (need_redraw) {
      ...
      put_dirty_first = ((put_dirty_first) < (col) ? (put_dirty_first) : (col));
      put_dirty_last = ((put_dirty_last) > (col + mbyte_cells) ? (put_dirty_last) : (col + mbyte_cells));
    }
...
{
  (__builtin_expect(!(put_dirty_row != -1), 0) ? __assert_rtn(__func__, "grid.c", 343, "put_dirty_row != -1") : (void)0);
  if (put_dirty_first < put_dirty_last) {
    if (set_cursor) {
      ui_grid_cursor_goto(put_dirty_grid->handle, put_dirty_row,
                          ((put_dirty_last) < (put_dirty_grid->cols - 1) ? (put_dirty_last) : (put_dirty_grid->cols - 1)));
    }
    ...
    put_dirty_first = 2147483647;
    put_dirty_last = 0;
  }
  put_dirty_row = -1;
  put_dirty_grid = ((void*)0);
}
  ...
  for (int row = start_row; row < end_row; row++) {
    ...
    if (dirty_last > dirty_first) {
      ...
      if (put_dirty_row == row) {
        put_dirty_first = ((put_dirty_first) < (dirty_first) ? (put_dirty_first) : (dirty_first));
        put_dirty_last = ((put_dirty_last) > (dirty_last) ? (put_dirty_last) : (dirty_last));
      } else if (grid->throttled) {

        (__builtin_expect(!(grid == &msg_grid), 0) ? __assert_rtn(__func__, "grid.c", 435, "grid == &msg_grid") : (void)0);
        int dirty = 0;
        ...
        ui_line(grid, row, dirty_first, last, dirty_last, attr, 0);
      }
    }

    if (end_col == grid->cols) {
      grid->line_wraps[row] = 0;
    }
  }
}
...
# 484 "/Users/foxacid/ghq/github.com/foxacid7cd/Nims/Targets/libNims/src/nvim/grid.c"
void grid_put_linebuf(ScreenGrid *grid, int row, int coloff, int endcol, int clear_width,
                      int rlflag, win_T *wp, int bg_attr, _Bool wrap)
{
  size_t max_off_from;
  size_t max_off_to;
  int col = 0;
  ...
  if (grid->chars == ((void*)0) || row >= grid->rows || coloff >= grid->cols) {
                                  ;
    return;
  }
...
void grid_alloc(ScreenGrid *grid, int rows, int columns, _Bool copy, _Bool valid)
{
  int new_row;
  ScreenGrid new = *grid;
  (__builtin_expect(!(rows >= 0 && columns >= 0), 0) ? __assert_rtn(__func__, "grid.c", 648, "rows >= 0 && columns >= 0") : (void)0);
  ...
  for (new_row = 0; new_row < new.rows; new_row++) {
    ...
    if (copy) {
      ...
      if (new_row < grid->rows && grid->chars != ((void*)0)) {
        int len = ((grid->cols) < (new.cols) ? (grid->cols) : (new.cols));
        __builtin___memmove_chk (new.chars + new.line_offset[new_row], grid->chars + grid->line_offset[new_row], (size_t)len * sizeof(schar_T), __builtin_object_size (new.chars + new.line_offset[new_row], 0));


        __builtin___memmove_chk (new.attrs + new.line_offset[new_row], grid->attrs + grid->line_offset[new_row], (size_t)len * sizeof(sattr_T), __builtin_object_size (new.attrs + new.line_offset[new_row], 0));
...
void grid_free(ScreenGrid *grid)
{
  ...
  grid->chars = ((void*)0);
  grid->attrs = ((void*)0);
  grid->line_offset = ((void*)0);
  grid->line_wraps = ((void*)0);
}
...
void win_grid_alloc(win_T *wp)
{
  ...
  _Bool want_allocation = ui_has(kUIMultigrid) || wp->w_floating;
  _Bool has_allocation = (grid_allocated->chars != ((void*)0));
...
void grid_ins_lines(ScreenGrid *grid, int row, int line_count, int end, int col, int width)
{
  ...
  for (i = 0; i < line_count; i++) {
    if (width != grid->cols) {
      ...
      j += line_count;
      grid_clear_line(grid, grid->line_offset[j] + (size_t)col, width, 0);
      grid->line_wraps[j] = 0;
    } else {
      j = end - 1 - i;
      ...
      grid->line_wraps[j + line_count] = 0;
      grid_clear_line(grid, temp, grid->cols, 0);
    }
  }
...
static void linecopy(ScreenGrid *grid, int to, int from, int col, int width)
{
  ...
  __builtin___memmove_chk (grid->chars + off_to, grid->chars + off_from, (size_t)width * sizeof(schar_T), __builtin_object_size (grid->chars + off_to, 0));
  __builtin___memmove_chk (grid->attrs + off_to, grid->attrs + off_from, (size_t)width * sizeof(sattr_T), __builtin_object_size (grid->attrs + off_to, 0));
}
