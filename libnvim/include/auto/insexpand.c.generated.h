...
static char_u *ins_compl_infercase_gettext(const char_u *str, int char_len, int compl_char_len, int min_len, char **tofree);
...
static int thesaurus_add_words_in_line(char *fname, char_u **buf_arg, int dir, const char_u *skip_word);
static void ins_compl_files(int count, char **files, int thesaurus, int flags, regmatch_T *regmatch, char_u *buf, Direction *dir) FUNC_ATTR_NONNULL_ARG(2, 7);
...
static char_u *ins_comp_get_next_word_or_line(buf_T *ins_buf, pos_T *cur_match_pos, int *match_len, _Bool *cont_s_ipos);
