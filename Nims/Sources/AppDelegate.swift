...
struct CmdlinesView: View {
  ...
  var body: some View {
    VStack(alignment: .center, spacing: 0) {
      ForEach(store.state.cmdlines) { cmdline in
        HStack {
          Spacer()
          ...
          VStack(alignment: .leading, spacing: 4) {
            if !cmdline.prompt.isEmpty {
              let attributedString = AttributedString(
                cmdline.prompt,
                attributes: .init([
                  .font: store.state.font.nsFont(isItalic: true),
                  .foregroundColor: store.state.appearance.defaultForegroundColor.appKit
            ...
            HStack(alignment: .firstTextBaseline, spacing: 10) {
              if !cmdline.firstCharacter.isEmpty {
                let attributedString = AttributedString(
                  ...
                  attributes: .init([
                    .font: store.state.font.nsFont(isBold: true),
                    .foregroundColor: store.state.appearance.defaultForegroundColor.appKit,
                  ])
                )
                Text(attributedString)
                  ...
                  .background(
                    store.state.appearance.defaultForegroundColor.swiftUI
                      .opacity(0.2)
                  )
              }
              ...
              ZStack(alignment: .leading) {
                ...
                let frame = integerFrame * store.state.font.cellSize

                Rectangle()
                  .fill(store.state.appearance.defaultForegroundColor.swiftUI)
          ...
          .background {
            ...
            rectangle
              .fill(
                store.state.appearance.defaultBackgroundColor.swiftUI.opacity(0.9)
              )

            rectangle
              .stroke(
                store.state.appearance.defaultForegroundColor.swiftUI.opacity(0.1),
                lineWidth: 1
              )
          }
      ...
      for blockLine in cmdline.blockLines {
        ...
        for contentPart in blockLine {
          let attributedString = AttributedString(
            ...
            attributes: .init([
              .font: store.state.font.nsFont(),
              .foregroundColor: store.state.appearance.foregroundColor(for: contentPart.highlightID).swiftUI,
              .backgroundColor: store.state.appearance.backgroundColor(for: contentPart.highlightID).swiftUI,
            ])
          )
