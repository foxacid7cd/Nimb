...
class AppDelegate: NSObject, NSApplicationDelegate, EventListener {
  ...
  private func startNvimProcess() {
    ...
    let nvimProcess = NvimProcess(
      ...
      mouseInput: self.mouseInputSubject,
    ...
    self <~ nvimProcess.run()
      ...
      .bind(with: self) {
        do {
          try $0.handle(notifications: $1)

        } catch {
          "nvim notifications handle failed"
  ...
  @MainActor
  private func handle(notifications: [NvimNotification]) throws {
