  ...
  func makeNvimKeyCode() -> String {
    let specialKey: String? = {
      if self.keyCode == kVK_Escape {
        ...
        return self.characters.lowercased().utf16.first?.specialKey
      }
    }()
    let modifier = self.modifierFlags.modifier

    let nvimNotatedKeyCode = [specialKey, modifier]
      .compactMap { $0 }
      .joined(separator: "-")

    if !nvimNotatedKeyCode.isEmpty {
      return "<\(nvimNotatedKeyCode)>"
    } else {
...
extension UTF16.CodeUnit {
  var specialKey: String? {
    UTF16.CodeUnit.SpecialKeys[Int(self)]
  }

  private static let SpecialKeys = [
    NSEnterCharacter: "CR",
    NSBackspaceCharacter: "BS",
    NSDeleteCharacter: "Del",
    NSTabCharacter: "Tab",
